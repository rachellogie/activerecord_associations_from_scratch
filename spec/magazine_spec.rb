...
feature "Magazine Subscriptions" do
  scenario "Can see the subscribers and their expiration dates for a magazine subscription" do
    ...
    other_magazine = Magazine.create!(name: "Home and Garden")
    bob = Subscriber.create!(name: "Bob Smith")
    sue = Subscriber.create!(name: "Sue Jones")
    don = Subscriber.create!(name: "Don Don't")
    ...
    don_expiration_date = 5.years.from_now
    Subscription.create!(magazine_id: other_magazine.id, subscriber_id: don.id, expires_on: don_expiration_date)
    ...
    within(".subscriptions") do
      ...
      expect(page).to have_no_content don_expiration_date.strftime("%F")
    end

    within(".subscribers") do
      expect(page).to have_content "Bob Smith"
      expect(page).to have_content "Sue Jones"
      expect(page).to have_no_content "Don Don't"
    end
  end
end